...
PretrainedBackboneType = (
    Literal[  # cannot unpack this directly in python < 3.11 so it has to be copied
...
# (but aggregation modes may be adapted eventually)
...
class PretrainedFeatureExtractorConfig:
    ...
    normalize_embeddings: bool = (
        True  # whether to normalize the embeddings (divide by the norm)
    )
    ...
    def _check_model_availability(self):
        if self.model_name not in AVAILABLE_PRETRAINED_BACKBONES.keys():
            raise ValueError(
                f"Model {self.model_name} is not available for feature extraction."
            )
        if self.model_name == "weigertlab/tarrow":
            if not TARROW_AVAILABLE:
                raise ImportError(
                    "TArrow is not available. Please install it to use this model."
                )
        ...
        if self.additional_features is not None:
            ...
            if self.additional_features not in wrfeat._PROPERTIES:
                raise ValueError(
                    f"Additional feature {self.additional_features} is not valid."
                )
...
class FeatureExtractor(ABC):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        self.batch_return_type: Literal[
        ...
        self.apply_rope = (
            False  # deprecated, use "rotate_features" in CTCData # TODO remove
        )
    ...
    def _load_model_from_path(self) -> tuple[torch.nn.Module, int]:
        ...
        raise NotImplementedError(
            "This model currently only supports being loaded from huggingface's hub."
        )
    ...
    def from_model_name(
        ...
        if model_name not in cls._available_backbones:
            raise ValueError(
                f"Model {model_name} is not available for feature extraction."
            )
        logger.info(
            f"Using model {model_name} with mode {mode} for pretrained feature extraction."
        )
    ...
    def from_config(
        cls,
        config: PretrainedFeatureExtractorConfig,
        image_shape: tuple[int, int],
        ...
        if config.model_name not in cls._available_backbones:
            raise ValueError(
                f"Model {config.model_name} is not available for feature extraction."
            )
        logger.info(
            f"Using model {config.model_name} with mode {config.mode} for pretrained feature extraction."
        )
    ...
    def compute_region_features(
        ...
        match self.mode:
            case "nearest_patch":
                feats = self._nearest_patches(
                    coords, masks, norm=self.normalize_embeddings
                )
            ...
            case mode if mode.endswith("_patches_exact"):
                if masks is None or labels is None or timepoints is None:
                    raise ValueError(
                        "Masks and labels must be provided for the chosen patch mode."
                    )
                feats_func = partial(
                    self._agg_patches_exact,
                    masks,
                    timepoints,
                    labels,
                    norm=self.normalize_embeddings,
                )
    ...
    def precompute_image_embeddings(self, images, **kwargs):  # , windows, window_size):
        ...
        if missing:
            for ts, batches in tqdm(
                self._prepare_batches(images),
                total=len(images) // self.batch_size,
    ...
    def _extract_region_embeddings(
        self, all_frames_embeddings, window, start_index, remaining=None
    ):
        ...
        n_regions_per_frame, features = self.extract_embedding(
            window_masks, window_timepoints, window_labels, window_coords
        )

        for i in range(remaining or len(n_regions_per_frame)):
            # if computing remaining frames' embeddings, start from the end
            obj_per_frame = (
                n_regions_per_frame[-i - 1] if remaining else n_regions_per_frame[i]
            )
            frame_index = (
                start_index + i if not remaining else np.max(window_timepoints) - i
            )
    ...
    def extract_embedding(self, masks, timepoints, labels, coords):
        ...
        if coords_txy.shape[0] != tot_regions:
            raise RuntimeError(
                f"Number of coords ({coords_txy.shape[0]}) does not match the number of coordinates ({timepoints.shape[0]})."
            )
        ...
        if tot_regions != features.shape[0]:
            raise RuntimeError(
                f"Number of regions ({n_regions_per_frame}) does not match the number of embeddings ({features.shape[0]})."
            )
    ...
    def _run_model(
        self, images, **kwargs
    ) -> torch.Tensor:  # must return (B, grid_size**2, hidden_state_size)
    ...
    def apply_rot_to_features(
        self, features: torch.Tensor, centroids: np.ndarray
    ) -> torch.Tensor:
        ...
        if torch.allclose(rotated, features):
            logger.warning(
                "Rotated features are equal to original features. Rotation may not be applied correctly."
            )
        return rotated
        ...
        for i in range(0, len(images), self.batch_size):
            ...
            if self.n_channels > 1:  # repeat channels if needed
                if self.orig_n_channels > 1 and self.orig_n_channels != self.n_channels:
                    raise ValueError(
                        "When more than one original channel is provided, the number of channels in the model must match the number of channels in the input."
                    )
    ...
    def _find_bbox_cells(self, regions: dict, cell_height: int, cell_width: int):
        ...
        for region in regions:
            ...
            patches = self._find_region_cells(
                minr, minc, maxr, maxc, cell_height, cell_width
            )
    ...
    def _debug_show_patches(self, embeddings, masks, coords, patch_idxs):
        ...
        e = e.reshape(
            -1, self.hidden_state_size, self.final_grid_size[0], self.final_grid_size[1]
        ).swapaxes(0, 1)
        ...
        for i, (t, y, x) in enumerate(patch_idxs):
            ...
            points[i * self.hidden_state_size : (i + 1) * self.hidden_state_size] = (
                np.tile(point, (self.hidden_state_size, 1))
            )
        ...
        logger.debug(
            f"Lost labels : {set(np.unique(masks)) - set(np.unique(masks_resized))}"
        )
    ...
    def _nearest_patches(self, coords, masks=None, norm=True, embs=None):
        ...
        try:
            ...
            logger.error(
                f"KeyError: {e} - Check if the timepoint exists in embeddings_dict."
            )
        except IndexError as e:
            # TODO improve handling of this error. Maybe check shape earlier
            logger.error(
                f"IndexError: {e} - Embeddings exist but do not have the correct shape. Did the model input size change ? If so, please delete saved embeddings and recompute."
            )
    ...
    def _agg_patches_bbox(
        self, masks, timepoints, labels, agg=torch.mean, norm=True, embs=None
    ):
        ...
        res = joblib.Parallel(n_jobs=8, backend="threading")(
            joblib.delayed(process_region)(i, t)
    ...
    def _agg_patches_exact(
        ...
        feats = torch.zeros(n_regions, self.hidden_state_size, device=self.device)
        embeddings = self._load_features() if embs is None else embs
        embeddings, grid_size = self._view_embeddings(
            embeddings, context={"masks_shape": masks.shape}
        )
        ...
        def process_region(i, t, masks):
            ...
            if not np.any(mask_reg):
                logger.warning(
                    f"No pixels found for region {labels[i]} at timepoint {t}."
                )
            ...
            patch_embeddings = FeatureExtractor.normalize_tensor(
                patch_embeddings, norm=norm
            )
            ...
            if patch_embeddings.shape[0] == 0:
                logger.warning(
                    f"No mapped pixels for region {labels[i]} at timepoint {t}."
                )
        ...
        if self.parallel:
            res = joblib.Parallel(n_jobs=8, backend="threading")(
                joblib.delayed(process_region)(i, t, masks=masks)
                for i, t in enumerate(timepoints_shifted)
            )
            for i, r in enumerate(res):
                # If agg is torch.max or torch.median, extract only the values
                if isinstance(r, torch.return_types.max) or isinstance(
                    r, torch.return_types.median
                ):
                    feats[i] = r.values
                else:
                    feats[i] = r
        ...
        if self.embeddings is None:
            ...
            if load_path.exists():
                ...
                if np.any(np.isnan(features)):
                    raise RuntimeError(
                        f"NaN values found in features loaded from {load_path}."
                    )
                # check feature shape consistency
                if (
                    features.shape[1]
                    != self.final_grid_size[0] * self.final_grid_size[1]
                    or features.shape[2] != self.hidden_state_size
                ):
                    logger.error(
                        f"Saved embeddings found, but shape {features.shape} does not match expected shape {('n_frames', self.final_grid_size[0] * self.final_grid_size[1], self.hidden_state_size)}."
                    )
                ...
                logger.info(
                    f"No saved embeddings found at {load_path}. Features will be computed."
                )
                return None
        else:
            return self.embeddings
    ...
    def _check_missing_embeddings(self):
        ...
        if features is None:
            ...
            logger.info(
                f"Embeddings for {self.model_name} already exist. Skipping embedding computation."
            )
        return False
...
class FeatureExtractorAugWrapper:
    ...
    def __init__(
        ...
        self.extractor.do_save = (
            False  # do not save intermediate features (augmented image embeddings)
        )
    ...
    def _check_existing(self):
        ...
        if not save_path.exists() or self.force_recompute:
            logger.debug(
                f"Augmentation zarr store {save_path} does not exist or force_recompute is True. Recomputing features."
            )
    ...
    def _compute(self, images, masks):
        ...
        embs = self.extractor.precompute_image_embeddings(
            images, image_shape=images_shape
        )
        ...
        if self._debug_view is not None:
            ...
            embs = embs.reshape(
                -1,
                self.extractor.final_grid_size[0],
                self.extractor.final_grid_size[1],
                self.extractor.hidden_state_size,
            )
            embs = np.moveaxis(embs, 3, 0)
            self._debug_view.add_image(embs, name="Embeddings", colormap="inferno")
            self._debug_view.add_image(
                images.cpu().numpy(), name="Images", colormap="viridis"
            )
    ...
    def _compute_original(self, images, masks):
        ...
        images, masks = self.aug_pipeline.preprocess(
            images, masks, normalize_func=self.extractor.normalize_array
        )
    ...
    def _compute_augmented(self, images, masks, n):
        ...
        assert im_shape == masks_shape, (
            f"Augmented images shape {im_shape} does not match augmented masks shape {masks_shape}."
        )
        if im_shape[-2:] != self.extractor.orig_image_size:
            ...
            if isinstance(self.extractor, CoTrackerFeatures):
                ...
                self.extractor.final_grid_size = (
                    im_shape[-2] // stride,
                    im_shape[-1] // stride,
                )
            if im_shape[-1] == 0 or im_shape[-2] == 0:
                raise ValueError(
                    f"Augmented images have invalid shape {im_shape}. Cannot extract features."
                )
    ...
    def _process_aug(self, n, images, masks, existing_aug_ids, existing_features_dict):
        try:
            if str(f"{n + 1}") in existing_aug_ids:
                logger.info(
                    f"Augmentation {n + 1} already exists. Skipping computation."
                )
                aug_feat_dict = existing_features_dict[str(n + 1)]["data"]
                aug_record = existing_features_dict[str(n + 1)]["metadata"]
            else:
                aug_feat_dict, aug_record = self._compute_augmented(
                    images, masks, n=n + 1
                )
            result = {
                "n": n,
    ...
    def compute_all_features(self, images, masks, clear_mem=True, n_workers=8) -> dict:
        ...
        if present:
            ...
            if len(existing_augs) == self.n_aug + 1:
                logger.info(
                    f"All {self.n_aug} augmentations + original already exist. Loading existing features."
                )
        ...
        if "0" not in existing_aug_ids:
            ...
            logger.info(
                "Original features already exist. Skipping computation for original features."
            )
        ...
        if (
            isinstance(self.extractor, CoTrackerFeatures)
            or isinstance(self.extractor, TAPFeatures)
            or isinstance(self.extractor, MicroSAMFeatures)
        ):
            ...
            logger.debug(
                f"Disabling parallel processing for {self.extractor.__class__.__name__} due to variable grid size."
            )

        if n_workers == 0 or disable_parallel:
            for n in range(self.n_aug):
                res = self._process_aug(
        ...
        if clear_mem:
            ...
            try:
                ...
                logger.error(
                    f"Model attribute not found: {e}. Skipping model transfer to CPU."
                )
            self.extractor = None
            if torch.cuda.is_available():
                torch.cuda.empty_cache()
    ...
    def _save_features(self, aug_id: int, aug_data: dict):
        ...
        for key, values in features_dict.items():
            features_group.create_dataset(
                key, data=np.stack(values), compressor=compressor
            )
    ...
    @staticmethod
    def load_features(
        path: str | Path, additional_props: str | None = None, n_jobs: int = 12
    ) -> dict:
        ...
        def _load_single(aug_id):
            ...
            try:
                ...
                if additional_props is not None:
                    missing_keys = [
                        k for k in required_features if k not in features_dict
                    ]
                    if missing_keys:
                        raise RuntimeError(
                ...
                logger.error(
                    f"KeyError: {e} - Augmentation {aug_id} is missing some data. Skipping."
                )
                missing = True
            if missing:
                return aug_id, None
            raise RuntimeError(
                f"Augmentation {aug_id} could not be loaded. Missing data or invalid format."
            )
        ...
        all_data = {
            aug_id: aug_data for aug_id, aug_data in results if aug_data is not None
        }
        return all_data
...
class SAMFeatures(FeatureExtractor):
    ...
    def _run_model(self, images, **kwargs) -> torch.Tensor:
        ...
        return outputs.permute(0, 2, 3, 1).reshape(
            B, H * W, N
        )  # (B, grid_size**2, hidden_state_size)
...
class SAM2Features(FeatureExtractor):
    ...
    def _run_model(self, images: list[np.ndarray], **kwargs) -> torch.Tensor:
        ...
        with torch.autocast(device_type=self.device), torch.inference_mode():
            images_ten = torch.stack([torch.tensor(image) for image in images]).to(
                self.device
            )
...
class CoTrackerFeatures(FeatureExtractor):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        self.final_grid_size = (
            image_size[0] // self.model.stride,
            image_size[1] // self.model.stride,
        )
    ...
    def precompute_image_embeddings(
        self, images, image_shape=None, **kwargs
    ):  # , windows, window_size):
        """Precomputes embeddings for all images."""
        try:
            if image_shape is None:
                ...
                if H != self.input_size[0] or W != self.input_size[1]:
                    ...
                    logger.debug(
                        f"Updated CoTracker input size: {self.input_size}, final grid size: {self.final_grid_size}"
                    )
    ...
    def _run_model(
        self, images: list[np.ndarray], image_shape=None, **kwargs
    ) -> torch.Tensor:
        self.model.eval()
...
class MicroSAMFeatures(FeatureExtractor):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        if not MICRO_SAM_AVAILABLE:
            raise ImportError(
                "microSAM is not available. Please install it following the instructions in the documentation."
            )
        super().__init__(image_size, save_path, batch_size, device, mode)
        self.input_size = 1024
    ...
    def _run_model(self, images: list[np.ndarray], **kwargs) -> torch.Tensor:
        ...
        for image in images:
            ...
            embedding = (
                self.model.get_image_embedding()
            )  # (1, hidden_state_size, grid_size, grid_size)
...
class TAPFeatures(FeatureExtractor):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        self.full_model, self.hidden_state_size = self._load_model_from_path(
            model_folder
        )
        self.full_model.to(device)
        self.full_model.eval()
        AVAILABLE_PRETRAINED_BACKBONES["weigertlab/tarrow"]["feat_dim"] = (
            self.hidden_state_size
        )
    ...
    def __init__(
        ...
        **kwargs,
        ...
        if not CELLPOSE_AVAILABLE:
            raise ImportError(
                "Cellpose is not available. Please install it following the instructions in the documentation."
            )
        super().__init__(image_size, save_path, batch_size, device, mode)
        self.input_size = 256
    ...
    def normalize_array(self, images_batch):
        ...
        for n, b in enumerate(images_batch):
            ...
            b_ = cp_transforms.convert_image(
                b, channel_axis=None, z_axis=None, do_3D=False
            )
...
class EncodedLabelsFeatures(FeatureExtractor):
    ...
    def _encode_labels(self, labels):
        ...
        for i in range(labels.shape[0]):
            label = labels[i]
            features[i] = np.array(
                [int(x) for x in np.binary_repr(label, width=self.hidden_state_size)],
                dtype=np.float32,
            )
    ...
    def fit_on_embeddings(self, embeddings_source_folders: list[str | Path]):
        ...
        for path in embeddings_paths:
            ...
            if N_samples > self.max_frames:
                logger.info(
                    f"Amount of loaded frames exceeds {self.max_frames} limit for PCA computation."
                )
                break
            else:
        ...
        self.fit(embeddings)
        logger.info(
            f"Fitted PCA model with {self.n_components} components on {N_samples} frames."
        )
